...
export default function AssignDelivery() {
  ...
  const fetchAssignmentData = useCallback(async () => {
    try {
      ...
      console.log(
        `Processed ${processedOrders.length} unique orders from ${rows.length} rows`
      );
      ...
      setError(
        `Failed to load data: ${err.message}. Please check your backend server.`
      );
  ...
  const getSuggestedRiders = () => {
    ...
    const areaRiders = allRiders.filter((rider) =>
      rider.assignedAreas.some((area) =>
        uniqueAreas.some(
          (orderArea) => area.includes(orderArea) || orderArea.includes(area)
        )
      )
    );
    ...
    const matchedTextRiders = textRiders
      .map((textRider) => {
        const actualRider = allRiders.find(
          (r) =>
  ...
  const getUniqueAreas = () => {
    const areas = [
      ...new Set(pendingOrders.map((order) => order.area).filter(Boolean)),
    ];
    return areas.sort();
  };
  ...
  const assignDeliveries = async () => {
    ...
    const selectedOrderDetails = pendingOrders.filter(
      (order) => order.selected
    );
    const totalValue = selectedOrderDetails.reduce(
    ...
    if (window.confirm(message)) {
      try {
        ...
        alert(
          `✅ Successfully assigned ${selectedOrders.length} order(s) to ${rider?.name}!`
        );
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-gray-800">
              🚚 Assign Delivery Duty
            </h1>
            <p className="text-gray-600 mt-2">
              Manage and assign deliveries to available riders
            </p>
            <div className="flex items-center gap-2 mt-1">
              ...
              <span className="text-sm text-gray-500">
                {pendingOrders.length} pending orders • {allRiders.length}{" "}
                riders available
              </span>
            </div>
          </div>
              ...
              <div className="flex-shrink-0">
                ...
                >
                  <path
                    ...
                    strokeLinejoin="round"
                    strokeWidth={2}
                    d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.998-.833-2.732 0L4.342 16.5c-.77.833.192 2.5 1.732 2.5"
                  />
                </svg>
              </div>
              <div className="ml-3">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-6">
              <div>
                ...
                <p className="text-gray-600 text-sm">
                  Select orders to assign for delivery
                </p>
                <p className="text-xs text-gray-500 mt-1">
                  Showing {filteredOrders.length} of {pendingOrders.length}{" "}
                  orders
            ...
            <div className="space-y-3 max-h-[500px] overflow-y-auto pr-2">
              {filteredOrders.length > 0 ? (
                filteredOrders.map((order) => {
                  const parsedRiders = parseAvailableRiders(
                    order.availableRidersText
                  );
          ...
          <div className="bg-white rounded-xl shadow-lg p-6">
            ...
            <div className="mb-4">
              <select
                ...
                disabled={
                  selectedOrders.length === 0 || showRiders.length === 0
                }
              >
                <option value="">
                  {showRiders.length === 0
              ...
              {selectedRider && (
                <div className="mt-3 p-3 bg-blue-50 rounded-lg border border-blue-200">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        ID: {selectedRider} • Mobile:{" "}
                        {allRiders.find((r) => r.id === selectedRider)?.mobile}
                      </p>
                    </div>
                  ...
                  {allRiders.find((r) => r.id === selectedRider)?.assignedAreas
                    .length > 0 && (
                    <div className="mt-2">
                      <p className="text-xs text-gray-500">Assigned Areas:</p>
                      <div className="flex flex-wrap gap-1 mt-1">
                        {allRiders
                          .find((r) => r.id === selectedRider)
                          ?.assignedAreas.slice(0, 3)
                          .map((area, idx) => (
                            <span
                              key={idx}
                              className="text-xs bg-gray-100 text-gray-700 px-2 py-1 rounded"
                        ...
                        {allRiders.find((r) => r.id === selectedRider)
                          ?.assignedAreas.length > 3 && (
                          <span className="text-xs bg-gray-200 text-gray-600 px-2 py-1 rounded">
                            +
                            {allRiders.find((r) => r.id === selectedRider)
                              ?.assignedAreas.length - 3}{" "}
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <span className="text-xs bg-gray-100 text-gray-700 px-2 py-1 rounded">
                            {rider.assignedAreas.length} area
                            {rider.assignedAreas.length !== 1 ? "s" : ""}
                          </span>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between">
          <div>
            <h3 className="font-bold text-gray-800">
              Delivery Assignment Ready
            </h3>
            <p className="text-gray-600 text-sm mt-1">
              {selectedOrders.length} orders • Rider:{" "}
              {selectedRider
                ? allRiders.find((r) => r.id === selectedRider)?.name
                : "Not selected"}{" "}
